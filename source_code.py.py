...
def main(tickers):    
    ...
    def get_pct_change(df):
        ...
        try:
            ...
            current = float(df.iloc[-1]['Adj Close'])
            previous = float(df[df.index == str(df.index[-1] - dt.timedelta(5))]['Adj Close'])
            week_diff = np.round(100*((current - previous) / previous),2)
        try:
            # Two weeks
            ...
            previous = float(df[df.index == str(df.index[-1] - dt.timedelta(10))]['Adj Close'])
            twoweek_diff = np.round(100*((current - previous) / previous),2)         
        ...
        try:
            ...
            previous = float(df[df.index == str(df.index[-1] - dt.timedelta(6*21))]['Adj Close'])
            sixmonth_diff = np.round(100*((current - previous) / previous),2)          
        ...
        try:
            ...
            previous = float(df[df.index == str(df.index[-1] - dt.timedelta(252))]['Adj Close'])
            oneyear_diff = np.round(100*((current - previous) / previous), 2)         
        ...
        try:
            ...
            previous = float(df[df.index == str(df.index[-1] - dt.timedelta(5*252))]['Adj Close'])
            fiveyear_diff = np.round(100*((current - previous) / previous), 2)
